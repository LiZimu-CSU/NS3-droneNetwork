        ...
        const std::chrono::microseconds& timeout)
{
    ...
    if (buffer_size > configuration()->sendBufferSize)
    {
        return false;
    }
    uint32_t send_buffer_size;
    octet* send_buffer;
    Locator remote_locator(locator);
    
    if (locator.address[12] == 0xef) //If it's multicast packet, let it go.
    {
    	send_buffer_size = buffer_size;
    	send_buffer = (octet *)malloc(send_buffer_size * sizeof(octet));
    	memcpy(send_buffer, buffer, buffer_size);
    }
    else{ //If it's only send to other IP, it will be sent to NS3 by multicast address 224.0.0.10 and port 9000
    	send_buffer_size = buffer_size+8;
    	send_buffer = (octet *)malloc(send_buffer_size * sizeof(octet));
    	unsigned char address[4] = {0xe0, 0x00, 0x00, 0x0a};
    	memcpy(remote_locator.address+sizeof(octet)*12, address, 4);
    	remote_locator.port = 9000;
    	memcpy(send_buffer, locator.address+sizeof(unsigned char)*12, 4);					//dest IP 
    	memcpy(send_buffer+sizeof(unsigned char)*4, &locator.port, 4);					//dest port 
    	memcpy(send_buffer+sizeof(unsigned char)*8, buffer, buffer_size);					//dds data
    }
