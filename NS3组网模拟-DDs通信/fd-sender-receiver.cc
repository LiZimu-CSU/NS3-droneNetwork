...
map<Ipv4Address, uint32_t> IpIdMap;
uint32_t copterID = 1;
...
void receivePacketFromNx(Ptr<Socket> sock){
    ...
    Ptr<Socket> sender = Socket::CreateSocket(uavs.Get(IpIdMap[temp]-1), tid);
    Ptr<Ipv4> ippp = uavs.Get(IpIdMap[temp]-1)-> GetObject<Ipv4> ();
    InetSocketAddress dest_addr = InetSocketAddress(ippp->GetAddress(1,0).GetLocal(), 20000);
    cout << "dest : " << IpIdMap[temp] << "  : " << ippp->GetAddress(1,0).GetLocal() <<endl ;
    ippp = uavs.Get(IpIdMap[sourece_address.GetIpv4()]-1)-> GetObject<Ipv4> ();
    InetSocketAddress source_addr_in_ns3 = InetSocketAddress(ippp->GetAddress(1,0).GetLocal(), 10000);
    cout << "source : " << IpIdMap[sourece_address.GetIpv4()] <<"  : " << ippp->GetAddress(1,0).GetLocal() <<'\n';
    sender->Bind(source_addr_in_ns3);//you wen ti
    sender->Connect(dest_addr);
    sender->Send(packet);
    sender->Close();
}
