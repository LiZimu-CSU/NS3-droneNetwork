...
int main(int argc, char *argv[])
{
    ...
    Ptr<Socket> recvMulti_socket = Socket::CreateSocket(receiver.Get(0), tid);
    InetSocketAddress addr33 = InetSocketAddress("239.255.0.1" , 20100);
...
// static Ipv6Address MakeIpv4MappedAddress (Ipv4Address addr);
    Ipv6Address v6Address = Ipv6Address::MakeIpv4MappedAddress (Ipv4Address("239.255.0.1"));

    Ipv4Address temp;
    temp = temp.Deserialize(dest_ip_port.ip);
    TypeId tid = TypeId::LookupByName("ns3::UdpSocketFactory");
    Ptr<Socket> sender = Socket::CreateSocket(uavs.Get(0), tid);
    InetSocketAddress dest_addr = InetSocketAddress("239.255.0.1", 20100);
